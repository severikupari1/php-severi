...
$Lisayspostitoimipaikka = mysqli_real_escape_string($conn, Tarkaste("Lisayspostitoimipaikka"));
 ...
 //poista if loppu
  ...
  if($osoite != "")
	...
	else
	{
		...
		//käydään tavarat läpi 
        echo "<table>";
        echo "<tr><th>Nimi</th><th>Osoite</th><th>Postinumero</th><th></th><th>Asiakas selite</th></tr>";
		while ($rivi = mysqli_fetch_array($tulos, MYSQL_ASSOC)) { 
			//haetaan nimi, hinta ja määrä muuttujiin 
			$avain = $rivi["avain"];
			$nimituloste = $rivi["nimi"]; 
 ...
 echo "<tr><td>$nimituloste</td><td>$osoitetuloste</td><td>$postinumerotuloste</td><td><a href=\"php-tehtavat.php?poista={$avain}\">Poista</a></td><td>$asikasselite</td><td><a href=\"php-tehtavat.php?muutosavain={$avain}\">Muokkaa</a></td></tr>";
...
('$Lisaysnimi', '$Lisaysosoite', '$Lisayspostinumero', '$Lisayspostitoimipaikka', '$timestamp', $lisaaasiakastyyppi)";
